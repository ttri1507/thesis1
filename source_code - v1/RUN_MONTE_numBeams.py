...
def run():
    ...
    numMonte = 100
    NumB = np.array([20, 25, 30, 35, 40], dtype=int)
    ee_total = np.zeros((4, len(NumB)), dtype=float)

    for numScen in range(len(NumB)):
        ...
        while numCurrent < numMonte:
            ...
            theta = 0.5
            model_rc = FUNC_randomClustering(model.copy(), int(NumB[numScen]), t_thres, theta, Area)
            id_clus_ue_rc = model_rc["IdClusUE"]
            numUE_in_SAT_rc = model_rc["NumUE_in_SAT"]
            model_gtc = FUNC_CoalitionGame(model.copy(), int(NumB[numScen]), t_thres, theta, Area, id_clus_ue_rc, numUE_in_SAT_rc)
            ...
            pow_gtcep = np.zeros((numSAT, numUE), dtype=float)
            ...
            id_clus_ue = model_rc["IdClusUE"]
            p_each_ue = Pmax / numUE_in_SAT
            pow_rcep = np.zeros((numSAT, numUE), dtype=float)
            for u in range(numUE):
                m = id_clus_ue[u]
