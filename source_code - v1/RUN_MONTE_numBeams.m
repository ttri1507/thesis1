...
% loop
numMonte = 100;
NumB = [20 25 30 35 40]; % The maximum beams per SAT


EE_total = zeros(4,length(NumB));

for numScen = 1:length(NumB)
...
% start_RC = tic;
[MODEL_RC] = FUNC_randomClustering(MODEL,NumB(numScen),t_thres,theta,Area);
...
start_GTC = tic;
[MODEL_GTC] = FUNC_CoalitionGame(MODEL,NumB(numScen),t_thres,theta,Area,IdClusUE_RC,NumUE_in_SAT_RC);
